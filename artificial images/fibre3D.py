...
class Fibre:
    def __init__(self, df, lf, point=(0, 0, 0), point_type='centroid',
                 phiDegxy=0, thetaDegxy=90, spcentre=(0, 0, 0)):
        self.df = df
        self.lf = lf
        self.phiDegxy = phiDegxy  # degrees
        self.phiDegij = 90 + phiDegxy  # degrees
        self.thetaDegxy = thetaDegxy  # degrees
        self.thetaDegij = 180 - thetaDegxy  # degrees
    ...
    def calc_uvec(self):
        ...
        theta_r = np.deg2rad(self.thetaDegij)
        s = np.sin(theta_r)
        uvec = np.array([s*np.cos(phi_r), s*np.sin(phi_r), np.cos(theta_r)])
        return uvec
    ...
    def translate(self, shift=(0, 0, 0)):

...
def map_rv2tan(x, pmf_x, y):
    ...
    """
    assert len(x) == len(pmf_x), print("check input: len(x) not equal to len(pmf_x).")
    F = interp1d(x, pmf_x, kind='linear', fill_value='extrapolate')
...
def map_rv2arctan(x, pmf_x, y):
    ...
    """
    assert len(x) == len(pmf_x), print("check input: len(x) not equal to len(pmf_x).")
    F = interp1d(x, pmf_x, kind='linear', fill_value=0)
...
def product_distr(x, y, z, pmfx, pmfy, atol=1e-4):
    ...
    fx = interp1d(x, pmfx, kind='linear', bounds_error=False, fill_value=(0, 0))
    pmfz = []
...
def ratio_distr(x, y, z, pmfx, pmfy, atol=1e-4):
    ...
    for k in z:
        ...
        # p_mskd[np.abs(p_mskd) < atol] = 0  # removing numerically very small values
        # p = np.trapz(p_mskd, y_mskd)  # integration (trapezoidal rule)
        p = np.abs(y) * pmfy * fx(k * y)
        p = np.trapz(p, y) # integration (trapezoidal rule)
        pmfz.append(p)
    return pmfz


def ratio_distr2(x, y, z, pmfx, pmfy, atol=1e-4):
    assert len(x) == len(pmfx)
    assert len(y) == len(pmfy)

    yvals = y
    yvals = np.append(yvals, atol)

    fy = interp1d(y, pmfy, kind='linear', bounds_error=False, fill_value=(0, 0))
    fx = interp1d(x, pmfx, kind='linear', bounds_error=False, fill_value=(0, 0))
    pmfz = []
    ...
    for k in z:
        ...
        # p_mskd[np.abs(p_mskd) < atol] = 0  # removing numerically very small values
        # p = np.trapz(p_mskd, y_mskd)  # integration (trapezoidal rule)
        p = np.abs(yvals)**100 * fy(yvals) * fx(k * yvals)
        p = simps(p, yvals)  # integration (trapezoidal rule)
        pmfz.append(p)
    return pmfz